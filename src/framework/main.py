...
parser.add_argument('--first_task_basemodel_folder', type=str, default='first_task_basemodel',
                    help="the folder name of first_task_basemodel")
...
parser.add_argument('--ini_path', type=str, default='',
                    help="The initial model path to accelerate the first base model "
                         "(some models are difficult to learn at the first training)")

# Architecture parameters
parser.add_argument('--multi_head', action="store_true",
                    help="Replace the final classifer for each task/center when stored, i.e., init_freeze = False")
...
parser.add_argument('--max_attempts_per_task', type=int, default=5,
                    help="Max amount of training attempts to stop infinite erroneous training.")
parser.add_argument('--no_maximal_plasticity_search', action="store_true",
                    help="do not use maximal plasticity search; instead a continuously decaying learning rate is used")
parser.add_argument('--accuracy_improvement_search', action="store_true",
                    help="do not use maximal plasticity search; instead try to reach improved accuracy")
...
parser.add_argument('--fixed_init_lr', type=float, default=0.001,
                    help="initial learning rate for optimizers like Adam, "
                         "all the lrs for later tasks/centers will be reloaded, 0.1 for SGD, 0.001 for Adam")
...
def main(method=None, dataset=None):
    ...
    if args.multi_head:
        args.init_freeze = False
    else:
        args.init_freeze = True

    if args.renew_optimizer:
    ...
    print('HYX 2', args.first_task_modelname)
    ...
    # INIT TASK
    prev_task_model_path = get_init_model_path(args, base_model, dataset, parent_exp_dir)
    ...
    for task_counter in range(args.starting_task_count, args.max_task_count * args.n_iters + 1):
...
class Manager(object):
    ...
    def set_dataset(self, args, rnd_transform=False):
        if hasattr(self.method, 'grid_datafetch'):  # Joint
            ...
            self.current_task_dataset_path = self.dataset.get_task_dataset_path_icl(
                task_name=args.task_name, rnd_transform=rnd_transform, total_tasks=args.n_tasks)
...
# CONFIG
########################################
def get_init_model_path(args, base_model, dataset, parent_exp_dir):
    if args.starting_task_count == 1:  # Starting from first task
        model_SI_path = utils.get_starting_model_path(args.tr_results_root_path, dataset, base_model.name,
                                                      args.first_task_modelname, method_name=methods.SI.name,
                                                      first_task_basemodel_folder=args.first_task_basemodel_folder,
                                                      append_filename=True)
        if args.train_first_task or args.first_task_basemodel_dump:  # Starting from scratch
            boost_path = utils.get_starting_model_path(args.tr_results_root_path, dataset, base_model.name,
                                                          args.ini_path, method_name=methods.SI.name,
                                                          first_task_basemodel_folder=args.ini_path)
            print("HYX", boost_path)
            if os.path.isfile(boost_path):
                prev_task_model_path = args.boost_path
                print("HYX: the model is initiated!")
            else:
...
def overwrite_dump_args(args, method):
    ...
    args.gridsearch_name = args.first_task_basemodel_folder
