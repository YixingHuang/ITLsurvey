...
class HyperparameterFramework(object):
    ...
    def getCurrentLearningRate(self, args):
        ...
        print("HYX, task counter", task_counter, current_lr)
        return current_lr
    ...
    def stabilityDecay(self, args, manager, finetune_lr, finetune_acc):
        ...
        if self.check_succes(manager):  # Skip this phase
            ...
            # manager.best_model_path = os.path.join(manager.heuristic_exp_dir, 'last_epoch.pth.tar')  # Set paths #HYX last model
            return
        ...
        while not converged and self.attempts < max_attempts:
            ...
            # CHECKPOINT
            if task_lr_acc > manager.best_acc:
                manager.best_acc = task_lr_acc
...
def framework_single_task(args, manager):
    ...
    #     manager.previous_task_model_path = 'C:/MachineLearning/CLsurveyICLdevelop/srclCLcopy/results/train/tiny_imgnet/SI/small_VGG9_cl_128_128/gridsearch/first_task_basemodel_ICL2/e=50_bs=200_lr=[0.0001, 0.0005, 0.001, 0.005, 0.01]/task_1/TASK_TRAINING/best_model.pth.tar'
    #     print('HYX, previous task model path ', manager.previous_task_model_path)
    # Init
    ...
    if args.task_counter > 1:
        ...
        previous_task_count = (args.task_counter - 2) % args.n_tasks + 1  # HYX
        prev_task_name = manager.dataset.get_taskname(previous_task_count)
    ...
    if args.no_maximal_plasticity_search:
        ...
        # if reload_optimier is Ture, only the initial learning rate is used,
        # all later learning rates will be overwritten by reloading the optimizer
        ft_lr = hf.getCurrentLearningRate(args)
        # ft_lr = args.fixed_init_lr
        print("*" * 20, " Inital learning rate is ", ft_lr, "*" * 20)
        if manager.best_acc is None:
            manager.best_acc = 0
        # HYX: the accuracy threshold is set to 0 for the first center
        hf.stabilityDecay(args, manager, ft_lr, manager.best_acc)
    elif args.accuracy_improvement_search:
        print("/nPHASE 2 (TASK {})".format(args.task_counter))
        # ft_lr = hf.getCurrentLearningRate(args, manager)  # HYX: get the learning with a smooth decaying schedule
        ...
        ##############################################################################
        # PHASE 1
        print("/nPHASE 1 (TASK {})".format(args.task_counter))
        ft_lr, ft_acc = hf.maximalPlasticitySearch(args, manager)
