...
def init(seed=7, deterministic=True):
    set_random(seed, deterministic=deterministic)
...
# DETERMINISTIC
########################################
def set_random(seed=7, deterministic=True):
...
def get_starting_model_path(root_path, dataset_obj, model_name, exp_name, method_name, first_task_basemodel_folder='first_task_basemodel', append_filename=True
                            ):
    """
    ...
    """
    print("HYX base", first_task_basemodel_folder)
    path = os.path.join(root_path, dataset_obj.train_exp_results_dir, method_name, model_name)
    path = os.path.join(path, 'gridsearch', first_task_basemodel_folder, exp_name, 'task_1', 'TASK_TRAINING')
...
def get_train_results_path(tr_results_root_path, dataset_obj, method_name=None, model_name=None, gridsearch=True,
    ...
    print("HYX,", tr_results_root_path, dataset_obj.train_exp_results_dir)
...
def attempt_copy(src_path, dest_path):
    try:
